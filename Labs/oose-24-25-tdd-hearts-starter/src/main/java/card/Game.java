...
public abstract class Game <ExtendedENum extends Enum<ExtendedENum>> {
    ...
    public void setPlayerToStart(int playerToStart){
    ...
    public void playARound(int playerToPlay) {
        int counter = 0;
        Player player = null;
        do {
            player = players.get(playerToPlay);
            playerPlaysARound(player);
            playerToPlay ++;
            playerToPlay = playerToPlay % getPlayersSize();
            counter ++;
        } while (counter < players.size() && !getFinishGame() && !playerHasWon(player));
    }
    ...
    public void playTillFinish(int plyerToStart){
        while (!finishGame && !getUser().hasWon()) {
            beforePlayOfRound();
            try {
                playARound(playerToStart);
            } catch  (Exception exp) {
                showPlayers();
                throw exp;
            }
